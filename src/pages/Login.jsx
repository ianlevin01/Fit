    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 16 }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ margin: '8px 0 0', color: 'var(--text-muted)' }}>Iniciá sesión para continuar</p>
        </div>
        ...
        <div className="card">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <button type="submit" className="btn-primary" style={{ width: '100%', marginTop: 8 }} disabled={isSubmitting}>
              {isSubmitting ? <><Loader2 size={16} className="spin" /> Entrando...</> : 'Iniciar sesión'}
            </button>
          </form>
        </div>

        <p style={{ textAlign: 'center', marginTop: 20, fontSize: 14, color: 'var(--text-muted)' }}>
          ¿No tenés cuenta? <Link to="/register" style={{ color: 'var(--accent)' }}>Registrate</Link>
        </p>
      </div>
    </div>
  );
